    ...
    def load_time_logs():
        ...
        if os.path.isfile("timelogs.json"):
            with open("timelogs.json") as file:
                ...
                time_logs = {key:defaultdict(lambda :{"duration":0,"time":get_time()},value) for key,value in time_logs.items()}
            return defaultdict(lambda :defaultdict(lambda :{"duration":0,"time":get_time()}, {"id":ID,"date":get_date()}),time_logs)
        return defaultdict(lambda :defaultdict(lambda :{"duration":0,"time":get_time()}, {"id":ID,"date":get_date()}), {})
